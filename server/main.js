...
var server=http.createServer(function(req,res){
    ...
    req.on("end",function(){
       try {
           ...
           res.write(JSON.stringify({error:"不是JSON数据"}));
           res.end();
           return;
       }
        ...
        switch (Number(data.type)) {
            ...
            case 0x02:
                if(!data.sf) return;
                for(var prop1 in cities[data.sf]){
                    arr.push(prop1);
                }
                break;
            case 0x03:
                if(!data.sf || !data.city || !cities[data.sf]) return;
                arr=cities[data.sf][data.city];
                break;
        }
        resObj.result=arr;
        res.writeHead(200,{
...
server.listen(4010,"10.60.15.184",function(){
    console.log("开启侦听");
});